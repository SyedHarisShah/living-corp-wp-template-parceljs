...
import searchlist from 'bundle-text:./list.eta'
...
export default class extends Page {
  ...
  async getJson(url){
    ...
    if(this.DOM.el.querySelectorAll('.el:not(.hide)').length==1){

      this.DOM.total.innerHTML = this.DOM.el.querySelectorAll('.el:not(.hide)').length+' result'
    }
    else{

      this.DOM.total.innerHTML = this.DOM.el.querySelectorAll('.el:not(.hide)').length+' results'
    }
