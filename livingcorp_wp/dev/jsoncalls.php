...
function getcpt($type,$tags,$pos){
  ...
  foreach($tags as $t){
    array_push($argspods['meta_query'], 
      array(
        'key'	 	=> 'tags',
        'value' => $t,
        'compare' => 'LIKE'
  ...
  if ($podsquery->have_posts()) : while ($podsquery->have_posts()) : $podsquery->the_post();
...
class gettags{
  ...
  public function gettags($object){
    ...
    if ($tagsquery->have_posts()) : while ($tagsquery->have_posts()) : $tagsquery->the_post();
      $object = (object) [
        'id' => get_the_ID(),
        'title' => get_the_title()
      ];
...
class gettopics{
    ...
    public function gettopics($object){
      ...
      foreach($top as $t){
        array_push($argstags['meta_query'], 
          array(
            'key'	 	=> 'topics_a',
            'value' => $t,
            'compare' => 'LIKE'
