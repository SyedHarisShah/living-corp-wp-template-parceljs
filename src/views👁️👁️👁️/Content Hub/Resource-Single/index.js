...
export default class extends Page {
  ...
  async generate(content) {
    ...
    // if(Difference_In_Days < numDayLimit && !this.main.user){
    if(false){
      ...
      html = Eta.render(template,{global:this.main,data:data,footer:this.footer, resource_link})
      // //  console.log(html)
    }
    document.querySelector('#content').innerHTML += html
    this.DOM = {
    ...
    if(Difference_In_Days < -14  && !this.main.user){
      ...
      await this.getRandom('resource',content.dataset.id,2)
