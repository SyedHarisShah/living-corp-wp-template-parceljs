...
import searchlist from 'bundle-text:/src/views👁️👁️👁️/ETA/searchlist.eta'
...
export default class extends Page {
  ...
  async generate(content) {
    ...
    html = Eta.render(template,{global:this.main,footer:this.footer})
  ...
  async blocksClick (el){
    ...
    if(this.DOM.el.querySelectorAll('.result:not(.hide)').length==1){

      this.DOM.total.innerHTML = this.DOM.el.querySelectorAll('.result:not(.hide)').length+' result'
    }
    else{

      this.DOM.total.innerHTML = this.DOM.el.querySelectorAll('.result:not(.hide)').length+' results'
    }
